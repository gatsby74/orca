...
import { DiscoveredSkillCard, skillSourceLabels } from './DiscoveredSkillCard'
        ...
        <div className="flex flex-wrap gap-2 text-[11px] text-muted-foreground">
          {(['home', 'repo', 'bundled', 'plugin'] as const).map((sourceKind) => (
            <span key={sourceKind} className="rounded-full border border-border px-2 py-1">
              {skillSourceLabels[sourceKind]} {sourceCounts[sourceKind]}
            </span>
          ))}
        </div>
      </section>
        ...
        {visibleSkills.length > 0 ? (
          <div className="mx-auto flex max-w-5xl flex-col gap-3">
            {visibleSkills.map((skill) => (
              <DiscoveredSkillCard key={skill.id} skill={skill} />
            ))}
          </div>
        ) : (
          <EmptyState
