...
export function SshPane(_props: SshPaneProps): React.JSX.Element {
  ...
  const loadTargets = useCallback(
    async (opts?: { signal?: AbortSignal }) => {
      try {
        ...
        if (!opts?.signal?.aborted && mountedRef.current) {
          toast.error(translate("auto.components.settings.SshPane.f1fc50dad2", "Failed to load SSH targets"))
        }
      }
    },
  ...
  const handleSave = async (): Promise<void> => {
    ...
    if (!host) {
      toast.error(translate("auto.components.settings.SshPane.0e5aa04161", "Host or SSH config alias is required"))
      return
    }

    if (isNaN(port) || port < 1 || port > 65535) {
      toast.error(translate("auto.components.settings.SshPane.4db9afce1c", "Port must be between 1 and 65535"))
      return
    }
    ...
    if (!isRelayGracePeriodValid(form, graceSeconds)) {
      toast.error(
        translate("auto.components.settings.SshPane.3879cbaa52", "Relay grace period must be between 60 and {{value0}} seconds, or choose keep alive until reset", { value0: MAX_SSH_RELAY_GRACE_PERIOD_SECONDS })
      )
      return
    }
    ...
    try {
      ...
      toast.success(editingId ? translate("auto.components.settings.SshPane.b4ba0ce33d", "Target updated") : translate("auto.components.settings.SshPane.f602009125", "Target added"))
      setShowForm(false)
      setEditingId(null)
      setForm(EMPTY_FORM)
      await loadTargets()
      ...
      if (mountedRef.current) {
        toast.error(err instanceof Error ? err.message : translate("auto.components.settings.SshPane.2227ce47b6", "Failed to save target"))
      }
    }
  }
  ...
  const handleRemove = async (id: string): Promise<void> => {
    try {
      ...
      if (mountedRef.current) {
        toast.error(err instanceof Error ? err.message : translate("auto.components.settings.SshPane.c2a69510e3", "Failed to remove target"))
      }
    }
  }
  ...
  const handleConnect = async (targetId: string): Promise<void> => {
    try {
      ...
      toast.error(err instanceof Error ? err.message : translate("auto.components.settings.SshPane.e95d5ae10e", "Connection failed"))
    }
  }
  ...
  const handleDisconnect = async (targetId: string): Promise<void> => {
    try {
      ...
      toast.error(err instanceof Error ? err.message : translate("auto.components.settings.SshPane.a43de1d3ee", "Disconnect failed"))
    }
  }
  ...
  const handleTerminateSessions = async (targetId: string): Promise<void> => {
    try {
      ...
      toast.success(translate("auto.components.settings.SshPane.90e308c98b", "Remote terminals ended"))
    } catch (err) {
      toast.error(err instanceof Error ? err.message : translate("auto.components.settings.SshPane.025e107643", "Failed to end remote terminals"))
    }
  }
  ...
  const handleResetRelay = async (targetId: string): Promise<void> => {
    try {
      ...
      if (mountedRef.current) {
        toast.error(err instanceof Error ? err.message : translate("auto.components.settings.SshPane.2c4ee7332b", "Failed to reset remote relay"))
      }
    }
  }
  ...
  const handleTest = async (targetId: string): Promise<void> => {
    ...
    try {
      ...
      if (mountedRef.current) {
        toast.error(err instanceof Error ? err.message : translate("auto.components.settings.SshPane.68c13b4589", "Test failed"))
      }
    } finally {
      if (mountedRef.current) {
        setTestingIds((prev) => {
    ...
    try {
      ...
      if (mountedRef.current) {
        if (synced.length === 0) {
          ...
          toast.success(translate("auto.components.settings.SshPane.f8050f6307", "Synced {{value0}} server{{value1}}", { value0: synced.length, value1: synced.length > 1 ? 's' : '' }))
        }
      }
      await loadTargets()
    } catch (err) {
      if (mountedRef.current) {
        toast.error(err instanceof Error ? err.message : translate("auto.components.settings.SshPane.f495689b82", "Import failed"))
      }
    }
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="space-y-0.5">
          <p className="text-sm font-medium">{translate("auto.components.settings.SshPane.94c5284560", "Targets")}</p>
          <p className="text-xs text-muted-foreground">
            {translate("auto.components.settings.SshPane.a7d28dff81", "Add a remote host to connect to it in Orca.")}</p>
        </div>
        <div className="flex shrink-0 items-center gap-1.5">
          ...
          >
            ...
            {translate("auto.components.settings.SshPane.51d7dba44d", "Import")}</Button>
          {!showForm ? (
            <Button
              variant="outline"
              size="xs"
            ...
            >
              ...
              {translate("auto.components.settings.SshPane.639ceb3698", "Add Target")}</Button>
          ) : null}
        </div>
      </div>
          ...
          <>
            ...
            {targets.length === 0 && !showForm ? (
              <div className="flex items-center justify-center rounded-lg border border-dashed border-border/60 bg-card/30 px-4 py-5 text-sm text-muted-foreground">
                {translate("auto.components.settings.SshPane.c0f1c80166", "No SSH targets configured.")}</div>
            ) : (
