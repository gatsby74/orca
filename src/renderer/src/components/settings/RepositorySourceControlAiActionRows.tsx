...
export function RepositorySourceControlAiActionRows({
  ...
  return (
    <div className="space-y-3">
      <Label className="text-xs font-medium">{translate("auto.components.settings.RepositorySourceControlAiActionRows.f0aa2cfaea", "Action recipes")}</Label>
          ...
          <div key={actionId} className="space-y-3 rounded-md border border-border px-3 py-3">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value={ACTION_MODE_INHERIT}>{translate("auto.components.settings.RepositorySourceControlAiActionRows.403876bb48", "Use global")}</SelectItem>
                  <SelectItem value={ACTION_MODE_OVERRIDE}>{translate("auto.components.settings.RepositorySourceControlAiActionRows.1cd88d470a", "Customize")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="grid gap-3 sm:grid-cols-[180px_1fr]">
              <div className="space-y-2">
                <Label className="text-[11px] text-muted-foreground">{translate("auto.components.settings.RepositorySourceControlAiActionRows.f4310cf63f", "Agent")}</Label>
                <Select
                  ...
                  <SelectContent>
                    <SelectItem value={DEFAULT_AGENT_VALUE}>
                      <span className="flex items-center gap-2">
                        ...
                        {translate("auto.components.settings.RepositorySourceControlAiActionRows.0ffb081b3a", "Use default agent")}</span>
                    </SelectItem>
                    {SOURCE_CONTROL_TEXT_ACTION_ID_SET.has(actionId) ? (
                      <SelectItem value={CUSTOM_AGENT_ID}>
                        <span className="flex items-center gap-2">
                          <Terminal className="size-3.5 text-muted-foreground" />
                          {translate("auto.components.settings.RepositorySourceControlAiActionRows.2b2f38652b", "Custom command")}</span>
                      </SelectItem>
                    ) : null}
                ...
                <Label className="text-[11px] text-muted-foreground">{translate("auto.components.settings.RepositorySourceControlAiActionRows.7a3a8e431d", "CLI arguments")}</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label className="text-[11px] text-muted-foreground">{translate("auto.components.settings.RepositorySourceControlAiActionRows.548a6e1281", "Command template")}</Label>
                <textarea
                  rows={3}
                  value={templateValue}
