  ...
  if (!automation) {
    return (
      <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
        {translate("auto.components.automations.AutomationDetail.221916d93c", "Create an automation to start scheduling agent work.")}</div>
    )
  }
  ...
  return (
    <div className="flex w-full flex-col gap-4">
      <div className="flex items-start justify-between gap-4 border-b border-border/50 pb-4">
        ...
        <div className="flex shrink-0 items-center gap-1">
          <Button variant="secondary" size="sm" onClick={() => onRunNow(automation)}>
            ...
            {translate("auto.components.automations.AutomationDetail.2fb1605beb", "Run Now")}</Button>
          <ToolbarIconButton label={translate("auto.components.automations.AutomationDetail.4b1ea02d2e", "Edit automation")} onClick={() => onEdit(automation)}>
            <Pencil className="size-4" />
          </ToolbarIconButton>
          <ToolbarIconButton
            label={automation.enabled ? translate("auto.components.automations.AutomationDetail.91a4155e95", "Pause automation") : translate("auto.components.automations.AutomationDetail.d79452fb30", "Resume automation")}
            onClick={() => onToggle(automation)}
          >
            {automation.enabled ? <Pause className="size-4" /> : <Play className="size-4" />}
          </ToolbarIconButton>
          <ToolbarIconButton
            label={translate("auto.components.automations.AutomationDetail.1f6026358e", "Delete automation")}
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(9rem,1fr))] gap-5 rounded-md border border-border/50 bg-muted/30 px-4 py-3 shadow-sm">
        <DetailMetric label={translate("auto.components.automations.AutomationDetail.18763ded26", "Schedule")} value={formatAutomationSchedule(automation.rrule)} />
        <DetailMetric
        ...
        />
        <DetailMetric label={translate("auto.components.automations.AutomationDetail.620b22145e", "Grace")} value={formatGrace(automation.missedRunGraceMinutes)} />
        <DetailMetric
        ...
        />
        <div className="min-w-0">
          <div className="text-[11px] font-medium uppercase text-muted-foreground">{translate("auto.components.automations.AutomationDetail.2df8970cd5", "Agent")}</div>
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(9rem,1fr))] gap-5 rounded-md border border-border/50 bg-muted/20 px-4 py-3 shadow-sm">
        ...
        />
        <DetailMetric label={translate("auto.components.automations.AutomationDetail.449fc83bf7", "Tokens")} value={formatAutomationTokens(usageSummary.totalTokens)} />
        <DetailMetric label={translate("auto.components.automations.AutomationDetail.a1d52c2189", "Usage coverage")} value={usageCoverage} />
      </div>

      <div className="rounded-md border border-border/50 bg-muted/20 shadow-sm">
        <div className="border-b border-border/50 px-3 py-2 text-sm font-medium">{translate("auto.components.automations.AutomationDetail.007c8ad874", "Prompt")}</div>
        <div className="px-3 py-3">
          <div className="min-w-0">
            <div className="text-[11px] font-medium uppercase text-muted-foreground">{translate("auto.components.automations.AutomationDetail.007c8ad874", "Prompt")}</div>
