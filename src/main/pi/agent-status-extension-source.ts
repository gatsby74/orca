...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  return [
    ...
    '// Why: Pi awaits extension event handlers. Keep hook delivery off the',
    '// agent critical path and bound loopback stalls so status reporting can',
    '// never pause a running Pi turn long enough to trip provider timeouts.',
    ...
    '  postQueue = postQueue',
    '    .then(() => postOnce(hookEventName, extra))',
    '    .catch(() => {})',
    '  // Why: keep the ordered background queue alive without making Pi await it.',
    '  void postQueue',
    '}',
    '',
    ...
    '    ? setTimeout(() => controller.abort(), HOOK_POST_TIMEOUT_MS)',
    '    : null',
